...
By default, GitHub Actions workflows run with elevated token permissions unless restricted.
- Always declare top-level `permissions: contents: read` (or the minimum permissions required) in your workflow configuration.
- Granting least-privilege permissions reduces the potential attack surface and prevents unauthorized write or administration access if a workflow job is compromised.
